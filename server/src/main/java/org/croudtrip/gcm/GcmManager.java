...
public class GcmManager {
	...
	public void sendDeclinePassengerMsg(JoinTripRequest request) {
		sendGcmMessageToUser(request.getSuperPassengerTrip().getQuery().getPassenger(), GcmConstants.GCM_MSG_REQUEST_DECLINED,
				new Pair<>(GcmConstants.GCM_MSG_REQUEST_DECLINED, "Your request was declined"),
				new Pair<>(GcmConstants.GCM_MSG_USER_MAIL, "" + request.getSuperPassengerTrip().getQuery().getPassenger().getEmail()),
	...
	public void sendAcceptPassengerMsg(JoinTripRequest request) {
		sendGcmMessageToUser(request.getSuperPassengerTrip().getQuery().getPassenger(), GcmConstants.GCM_MSG_REQUEST_ACCEPTED,
				new Pair<>(GcmConstants.GCM_MSG_USER_MAIL, "" + request.getSuperPassengerTrip().getQuery().getPassenger().getEmail()),
	...
	public void sendJoinTripRequestExpiredToPassenger(JoinTripRequest joinTripRequest) {
		sendGcmMessageToUser(joinTripRequest.getSuperPassengerTrip().getQuery().getPassenger(), GcmConstants.GCM_MSG_REQUEST_EXPIRED,
				new Pair<>(GcmConstants.GCM_MSG_USER_MAIL, "" + joinTripRequest.getSuperPassengerTrip().getQuery().getPassenger().getEmail()),
    ...
    public void sendArrivalTimeUpdate(JoinTripRequest request) {
        sendGcmMessageToUser(request.getSuperPassengerTrip().getQuery().getPassenger(), GcmConstants.GCM_MSG_ARRIVAL_TIME_UPDATE,
                new Pair<>(GcmConstants.GCM_MSG_JOIN_REQUEST_ID, "" + request.getId()));
    }
}
